...
目前內建 **107 種人工整理配方**：原有 6 種核心配方、新增的 100 種常見項目，以及經典史萊姆配方。沒有收錄的安全成品，仍可交由伺服器端 AI 產生結構化配方草稿。
...
- 手機、平板與桌面響應式介面
- PWA 安裝與完整人工圖鑑離線快取
- GitHub Actions 自動檢查 107 種配方的數量、唯一性與必要欄位
- GitHub Pages 靜態版與 Vercel AI 完整版

## 史萊姆配方安全設計

經典史萊姆使用 PVA 白膠、小蘇打，以及標示含 `boric acid` 或 `sodium borate` 的隱形眼鏡沖洗液。配方明確提醒：

- 不可使用過氧化氫型隱形眼鏡清潔液。
- 不可使用工業膠、瞬間膠或不明化學品替代。
- 不可食用，幼童需由成人全程陪同。
- 皮膚敏感、破皮或濕疹者應避免接觸。
- 出現刺激、異味、霉點或髒污時立即停用並丟棄。
...
瀏覽器開啟 `http://localhost:8000`。純靜態伺服器可使用完整 107 種人工圖鑑，但不會執行 `api/generate.js`。
...
- `.github/workflows/validate.yml` 會驗證 JavaScript、JSON、107 種唯一配方與三格圖解。
